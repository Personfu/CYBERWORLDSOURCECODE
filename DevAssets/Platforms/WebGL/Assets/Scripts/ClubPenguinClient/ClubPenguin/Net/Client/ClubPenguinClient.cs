...
namespace ClubPenguin.Net.Client
{
    ...
    // [PATCH] WebGL: Run on main thread because System.Threading is not supported.
    // Yield one frame to ensure UI updates before the heavy calculation.
    yield return null; 
    ...
    {
        UnityEngine.Debug.LogError("RSA Key Gen Failed: " + ex.Message);
        if (failureCallback != null) failureCallback();
        yield break;
    }
#else
            // Original threaded logic for Standalone/Editor
            ...
            // Wait until parameters are populated
