...
namespace Disney.Kelowna.Common
{
	public class WwwBundleDevice : Device
	{
		...
		private IEnumerator waitForBundleToLoad<TAsset>(AssetBundleWwwWrapper bundleRequestWrapper, uint crc, AssetLoadedHandler<TAsset> handler) where TAsset : class
		{
			...
			Debug.Log($"Manifest lookup done for: {bundleRequestWrapper.BundlePath}");

			if (string.IsNullOrEmpty(cpipeManifestResponse.FullAssetUrl))
			{
				Debug.LogError($"Bundle \"{bundleRequestWrapper.BundlePath}\" NOT FOUND in CPipe manifest.");
				throw new Exception(string.Format("Bundle \"{0}\" NOT FOUND in CPipe manifest.", bundleRequestWrapper.BundlePath));
			}

			// 2. Wait for caching to be ready
			//while (!Caching.ready)
			//{
			//	yield return null;
			//}
			...
			Debug.Log("Initial bundle download attempt finished.");
			...
			if (DelayLoading)
			{
				yield return null;
				yield return null;
				Debug.Log("DelayLoading applied.");
			}
			...
			for (int i = 0; i < 3; i++)
			{
				...
				else
				{
					...
					{
						Debug.Log("Bundle successfully downloaded and loaded.");
						break;
					}
