...
namespace ClubPenguin.Net.Client
{
    ...
    public class BaseCatalogSectionOperation<T> : CPAPIHttpOperation
    {
        ...
        protected override void PerformOfflineAction(OfflineDatabase offlineDatabase, IOfflineDefinitionLoader offlineDefinitions)
        {
            ...
            {
                if (requestedChallengeId <= 0L)
                {
                    items.AddRange(data.MarketplaceItems);
                }
                else
                {
            ...
            else if (section.Equals("recent", StringComparison.OrdinalIgnoreCase) || section.Equals("friends", StringComparison.OrdinalIgnoreCase))
            {
                items.Sort((a, b) => b.equipment.dateTimeCreated.CompareTo(a.equipment.dateTimeCreated));
            }
            else if (section.Equals("category", StringComparison.OrdinalIgnoreCase))
            {
                items.Sort((a, b) => b.equipment.dateTimeCreated.CompareTo(a.equipment.dateTimeCreated));
            }
            else
