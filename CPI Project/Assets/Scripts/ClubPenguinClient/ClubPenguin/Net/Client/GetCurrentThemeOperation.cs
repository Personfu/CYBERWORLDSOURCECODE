...
namespace ClubPenguin.Net.Client
{
    ...
    public class GetCurrentThemeOperation : CPAPIHttpOperation
    {
        ...
        {
            ClothingCatalogOfflineData data = offlineDatabase.Read<ClothingCatalogOfflineData>();
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                int dayStamp = ClothingCatalogOfflineData.GetDayStampUtc(day);
                long scheduleId = GetScheduleIdForDayStamp(dayStamp);

                long userSubmissionId = 0L;
                if (data.SubmittedItemByChallenge != null && data.SubmittedItemByChallenge.TryGetValue(scheduleId, out long subId))
        ...
        private static long GetScheduleIdForDayStamp(int dayStampUtc)
        {
            EnsureScheduleCaches();

            if (cachedScheduleIdByDayStampUtc != null && cachedScheduleIdByDayStampUtc.TryGetValue(dayStampUtc, out long id) && id > 0L)
                return id;

            if (cachedScheduleIds != null && cachedScheduleIds.Count > 0)
            {
                int idx = Math.Abs(dayStampUtc.GetHashCode()) % cachedScheduleIds.Count;
                return cachedScheduleIds[idx];
            }
        ...
        {
            if (cachedScheduleIds != null && cachedScheduleIds.Count > 0 && cachedScheduleIdByDayStampUtc != null && cachedScheduleIdByDayStampUtc.Count > 0)
                return;
            ...
            for (int i = cachedScheduleIds.Count - 1; i >= 0; i--)
            {
                long v = cachedScheduleIds[i];
                for (int j = 0; j < i; j++)
                {
                    if (cachedScheduleIds[j] == v)
                    {
                        cachedScheduleIds.RemoveAt(i);
                        break;
                    }
                }
            }
