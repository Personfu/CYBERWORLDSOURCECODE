    ...
    public class GetUserStatsOperation : CPAPIHttpOperation
    {
        ...
        protected override void PerformOfflineAction(OfflineDatabase offlineDatabase, IOfflineDefinitionLoader offlineDefinitions)
        {
            ...
            if (data.MarketplaceItems != null && data.MarketplaceItems.Count > 0)
            {
                ...
                if (data.SubmittedItemByChallenge != null && data.SubmittedItemByChallenge.TryGetValue(currentChallengeId, out long submittedId))
                {
                    for (int i = 0; i < data.MarketplaceItems.Count; i++)
                    {
                        if (data.MarketplaceItems[i].clothingCatalogItemId == submittedId)
                        {
                            currentItems.Add(data.MarketplaceItems[i]);
                            break;
                        }
                    }
                }
