...
namespace ClubPenguin.Locomotion
{
	public class RuntimeAudioParams : MonoBehaviour
	{
		...
		// Precompute Stopping tag hash for performance
			...
			while (true)
			{
				if (isActiveAndEnabled)
				{
					...
					if (LocomotionUtils.IsLocomoting(stateInfo))
					{
						...
						else if (stateInfo.tagHash == StoppingTagHash)
						{
							if (locoStatus != LocomotionStatus.Stopping || surfaceIndex != prevSurfaceTypeIndex)
							{
