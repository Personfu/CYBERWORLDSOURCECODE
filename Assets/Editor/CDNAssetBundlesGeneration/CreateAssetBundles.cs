...
public class CreateAssetBundles : MonoBehaviour
{
    ...
    static void BuildAllAssetBundles(BuildTarget target, ref Dictionary<string, string> bindings)
    {
        ...
        if ((int)target == 24)
        {
            assetBundleDirectory = "Export/ClientBundles/standalonelinux64";
        }
        else
        {
        ...
        foreach (string name in names)
        {
            ...
            if (!name.StartsWith("CDN/", System.StringComparison.OrdinalIgnoreCase))
                continue;
    ...
    static void GetNames()
    {
        ...
        foreach (BuildTarget target in targets)
        {
            ...
            if ((int)target == 24)
            {
                targetPath = string.Format("Assets/assetpipeline/generated/output/unbundledassets/test event/standalonelinux64");
            }
            else
            {
            ...
            {
                writer.WriteLine(string.Format("baseuri:ClientBundles/{0}", target.ToString().ToLower()));
            }
    ...
    static void GenerateManifest()
    {
        ...
        foreach (BuildTarget target in targets)
        {
            ...
            if ((int)target == 24)
            {
                clientManifestDirectory.Add(new ClientManifest("1.13.0", target.ToString().ToLower(), "production", "Client 1.13.0_2018_11_05", false, string.Format("ClientBundles/standalonelinux/{1}.unity3d", target.ToString().ToLower(), cmanifestName), "", generateHexString(), "Client 1.13.0", "2025-4-7 00:00:00 -08:00"));
            }
            else
            {
                clientManifestDirectory.Add(new ClientManifest("1.13.0", target.ToString().ToLower(), "production", "Client 1.13.0_2018_11_05", false, string.Format("ClientBundles/{0}/{1}.unity3d", target.ToString().ToLower(), cmanifestName), "", generateHexString(), "Client 1.13.0", "2025-4-7 00:00:00 -08:00"));
            }
